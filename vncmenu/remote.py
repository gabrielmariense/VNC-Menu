...
def _decode_marked_payload(output, begin_marker, end_marker):
    ...
    O computador remoto comprime (deflate cru) e quebra a saida em linhas. O
    fallback sem compressao existe para o caso de uma versao antiga do payload
    ainda estar rodando em algum lugar, e custa uma tentativa.
    """
    padrao = (
    ...
    match = re.search(padrao, str(output or ""))
    if not match:
        return None
    try:
        bruto = base64.b64decode(re.sub(r"\s+", "", match.group(1)))
    except Exception:
        return None
    for tentativa in (lambda: zlib.decompress(bruto, -15), lambda: bruto):
        try:
            return json.loads(tentativa().decode("utf-8-sig"))
        except Exception:
            continue
    return None
...
def _truncation_error(host, psexec_path, returncode, stdout, stderr):
    ...
    return PsExecQueryError(
        ...
        "O comando rodou e começou a responder, mas a saída foi cortada no "
        "meio. Tente novamente; se repetir sempre nessa máquina, os detalhes "
        "técnicos mostram quantos caracteres chegaram, que é o que identifica "
        "onde ela está sendo cortada.",
        _build_psexec_details(host, psexec_path, returncode, stdout, stderr)
        + f"\n\nCaracteres recebidos em STDOUT: {len(str(stdout or ''))}",
...
def parse_run_script_payload(output: str) -> dict:
    ...
    data = _decode_marked_payload(
        output, "__VNC_MENU_RUNVBS_BEGIN__", "__VNC_MENU_RUNVBS_END__")
...
def parse_driver_install_payload(output: str) -> dict:
    ...
    data = _decode_marked_payload(
        output, "__VNC_MENU_DRIVERS_BEGIN__", "__VNC_MENU_DRIVERS_END__")
    return data if isinstance(data, dict) else {}
...
def _wmi_degradado(drivers: dict, script_data: dict) -> bool:
    """True quando algo so funcionou pelo caminho alternativo.

    Tres sinais, mesma causa: o provedor WMI da maquina nao respondeu. Vale
    aparecer no resumo porque a maquina esta degradada de um jeito que afeta
    mais coisas que a impressora - inclusive o proprio script da empresa, que
    nao consegue apagar as filas antigas nessas estacoes.
    """
    if str((script_data or {}).get("MetodoUsuario") or "") == "explorer_token":
        return True
    if str((script_data or {}).get("MetodoTarefa") or "") == "com":
        return True
    resultados = (drivers or {}).get("Results")
    if isinstance(resultados, list):
        for item in resultados:
            if isinstance(item, dict) and item.get("Fonte") == "instalou_com":
                return True
    return False
...
def format_run_summary(host: str, drivers: dict, script_data: dict) -> str:
    ...
    O relatorio completo continua existindo e vai para a janela de detalhes.
    Aqui fica o que muda a acao de quem esta atendendo: deu certo, quais filas,
    e o que fugiu do normal. Caminho, codigo da tarefa, tempos e por qual API
    cada coisa passou sao diagnostico - so ajudam depois que algo deu errado.
    """
    status_drivers = str((drivers or {}).get("Status") or "")
    status_script = str((script_data or {}).get("Status") or "")

    # Falha na fase de drivers: o script nem chegou a rodar.
    if status_drivers != "ok":
        linhas = [
            DRIVER_INSTALL_STATUS.get(
                status_drivers, f"Resultado não reconhecido: {status_drivers or '(vazio)'}"),
            "",
            "O script NÃO foi executado — executar sem os drivers apagaria as "
            "impressoras do usuário sem garantia de remapear.",
        ]
        dica = DRIVER_INSTALL_HINT.get(status_drivers, "")
        if dica:
            linhas.extend(["", dica])
        return "\n".join(linhas)
    ...
    if not script_data:
        return "\n".join([
            "A instalação de drivers terminou, mas o script não chegou a rodar.",
            "",
            "Abra os detalhes técnicos para ver o que aconteceu.",
        ])

    # Falha na execucao: a mensagem e a dica sao a acao, entao ficam aqui.
    if status_script != "ok":
        linhas = [
            f"Não foi possível executar em {host}.",
            SCRIPT_RUN_STATUS.get(
                status_script, f"Resultado não reconhecido: {status_script or '(vazio)'}"),
        ]
        dica = SCRIPT_RUN_HINT.get(status_script, "")
        if dica:
            linhas.extend(["", dica])
        detalhe = str((script_data or {}).get("Detail") or "").strip()
        if detalhe:
            linhas.extend(["", f"Mensagem do Windows: {detalhe}"])
        return "\n".join(linhas)

    linhas = [f"Impressoras atualizadas em {host}.", ""]

    resultados = (drivers or {}).get("Results")
    filas = (drivers or {}).get("Queues")
    if isinstance(resultados, list) and resultados:
        for item in resultados:
            if not isinstance(item, dict):
                continue
            ...
            if not item.get("Ok"):
                erro = str(item.get("Error") or "").strip()
                nota = f"   FALHOU{': ' + erro if erro else ''}"
            elif item.get("Fonte") in ("instalou", "instalou_com"):
                nota = "   driver instalado agora"
            elif item.get("JaExistia"):
                nota = "   já existia no servidor"
            else:
                nota = ""
            linhas.append(f"  {caminho}{nota}")
    elif isinstance(filas, list) and filas:
        linhas.extend(f"  {fila}" for fila in filas)

    usuario = str((script_data or {}).get("User") or "").strip()
    linhas.extend(["", f"Usuário: {usuario or '-'}"])

    if _wmi_degradado(drivers, script_data):
        linhas.extend([
            "",
            "Esta máquina está com o WMI danificado: os comandos passaram por "
            "um caminho alternativo. O script da empresa também não consegue "
            "apagar as filas antigas aqui.",
        ])

    linhas.extend(["", "Confirme pelo botão Impressoras."])
    return "\n".join(linhas)
